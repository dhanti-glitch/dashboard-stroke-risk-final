    ...
    if predict_btn and ai_ready:
        ...
        with col_r2:
            ...
            fig_bar.update_layout(height=320, **PLOTLY_LAYOUT,
                                  ...
                                  xaxis=dict(range=[0, factors["Kontribusi"].max() * 1.35]),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        # Konfigurasi per kolom: (nbins, xbins, tickvals, tickangle)
        col_cfg = {
            'age':               dict(nbins=None, xbins=dict(start=0, end=85, size=5),
                                      tickvals=[0,20,40,60,80], tickangle=0),
            'avg_glucose_level': dict(nbins=None, xbins=dict(start=50, end=280, size=15),
                                      tickvals=[50,100,150,200,250], tickangle=0),
            'bmi':               dict(nbins=None, xbins=dict(start=10, end=100, size=10),
                                      tickvals=[10,20,30,40,50,60,70,80,90], tickangle=0),
        }
        ...
        for i, col in enumerate(num_cols):
            ...
            fig.update_traces(xbins=cfg['xbins'])
            fig.update_layout(
                height=300, **PLOTLY_LAYOUT, bargap=0.05,
                margin=dict(t=40, b=55, l=30, r=10),
                showlegend=(i==0),
                xaxis=dict(
                    tickvals=cfg['tickvals'],
                    tickangle=cfg['tickangle'],
                    tickmode='array'
                )
            )
        ...
        <b>{mean_nonstroke:.1f}</b>. Pasien stroke <b>{direction} {diff_pct:.1f}%</b> dibanding non-stroke.</p></div>""", unsafe_allow_html=True)
